...
public class CustomScrollView extends ScrollView {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mOnScrollChangeListener != null) {
                    mOnScrollChangeListener.onActionDown();
                }
                Log.e("onInterceptHoverEvent", "ACTION_DOWN 按下========");
            default:
                break;
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_UP:
                Log.e("scrollview", "ACTION_UP 抬起");
                int scrollViewHeight = getMeasuredHeight();
                int layoutHeight = getChildAt(0).getMeasuredHeight();
                //不超过当前屏幕，并且有滑动过，就显示
                if (scrollViewHeight == layoutHeight) {
                    if (isScrollMoved) {
                        Log.e("scroll view", "不超过当前屏幕，并且有滑动过，显示");

                        if (mOnScrollChangeListener != null) {
                            mOnScrollChangeListener.onActionUp();
                        }
                    }
            ...
            case MotionEvent.ACTION_DOWN:
                Log.e("scroll view", "ACTION_DOWN 按下");
                if (mOnScrollChangeListener != null) {
                    mOnScrollChangeListener.onActionDown();
                }
                break;
            ...
            case MotionEvent.ACTION_MOVE:
                isScrollMoved = true;
                if (mOnScrollChangeListener != null) {
                    mOnScrollChangeListener.onActionMove();
                }
                break;
            default:
                break;
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (mOnScrollChangeListener != null) {
//            mOnScrollChangeListener.onTouch();
//            Log.i("CustomScrollView", "scrollY:" + getScrollY());
            ...
            if (getScrollY() == 0) {
                ...
                if (!isScrollToStart) {
                    ...
                    mHandler.sendEmptyMessageDelayed(CODE_TO_START, 100);
                ...
                if (contentView != null && contentView.getMeasuredHeight() == (getScrollY() + getHeight())) {
                    ...
                    if (!isScrollToEnd) {
                        isScrollToEnd = true;
                        mHandler.sendEmptyMessageDelayed(CODE_TO_END, 100);
